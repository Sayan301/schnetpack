...
class AtomisticDenoise(pl.LightningModule):
    """
    ...
    def __init__(
        ...
        warmup_steps: int = 0,
        pretrain_labels: Optional[Dict[str, Any]] = None,
    ...
    def training_step(self, batch, batch_idx):
        ...
        # except:
        #     pass
        targets ={'scalar_representation': batch['noise']}
        pred = self.predict_without_postprocessing(batch)
        pred['scalar_representation'] = pred['scalar_representation'].flatten()
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # print("Targets = ", targets['scalar_representation'].shape)
        pred = self.predict_without_postprocessing(batch)
        pred['scalar_representation'] = pred['scalar_representation'].flatten()
        # print("Predictions = ", pred['scalar_representation'].shape)
        # pred, targets = self.apply_constraints(pred, targets)
        ...
        loss_value = "{:.6f}".format(loss.item())
        with open(self.logfile, 'a') as logfile:
            writer = csv.DictWriter(logfile, fieldnames = ['epoch', 'val_loss'])
            writer.writeheader()
            writer.writerows([{'epoch': self.epoch, 'val_loss': loss_value}])
            self.epoch += 1
...
class AtomisticPretrain(pl.LightningModule):
    """
    The basic learning task in SchNetPack, which ties model, loss and optimizer together.
    ...
    def get_truelabels(self, batch):

        truelabels = torch.tensor([self.pretrain_labels[str(item.item())] for item in batch['_atomic_numbers']])
        return truelabels
    ...
    def training_step(self, batch, batch_idx):
        ...
        # except:
        #     pass
        targets ={'scalar_representation': self.get_truelabels(batch).to('cuda')}
        pred = self.predict_without_postprocessing(batch)
        # print("Targets = ", targets)
        # print("Predictions = ", pred)
        # pred, targets = self.apply_constraints(pred, targets)
